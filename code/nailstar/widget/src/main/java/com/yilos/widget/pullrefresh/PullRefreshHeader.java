...
public class PullRefreshHeader extends FrameLayout implements PtrUIHandler {
    ...
    private PullRefreshHeader.LastUpdateTimeUpdater mLastUpdateTimeUpdater = new PullRefreshHeader.LastUpdateTimeUpdater();
    ...
    public PullRefreshHeader(Context context, AttributeSet attrs, int defStyle) {
    ...
    private class LastUpdateTimeUpdater implements Runnable {
        ...
        private void start() {
            if (!TextUtils.isEmpty(PullRefreshHeader.this.mLastUpdateTimeKey)) {
                this.mRunning = true;
                this.run();
            }
        }
        ...
        private void stop() {
            this.mRunning = false;
            PullRefreshHeader.this.removeCallbacks(this);
        }

        public void run() {
            PullRefreshHeader.this.tryUpdateLastUpdateTime();
            if (this.mRunning) {
                PullRefreshHeader.this.postDelayed(this, 1000L);
            }
