...
import com.yilos.nailstar.player.entity.TopicsCommentAtInfo;
...
public class TopicsServiceImpl implements ITopicsService {
    @Override
    public TopicsInfo getTopicsInfo(String topicsId) throws IOException, JSONException, NetworkDisconnectException {
        ...
        String url = "/vapi/nailstar/topics/" + topicsId;
        //HttpClient.getJson(url);
        TopicsInfo topicsInfo = null;

        if (null != strVideoInfo && strVideoInfo.length() > 0) {
            topicsInfo = new TopicsInfo();
            ...
            for (int i = 0; i < jsonVideos.length(); i++) {
                ...
                videos.add(new VideoInfo(jsonVideoObj.optString("videoId", null), jsonVideoObj.optInt("playTimes", 0), jsonVideoObj.optString("ccUrl", null), jsonVideoObj.optString("ossUrl", null)));
            }


            topicsInfo.setId(jsonResult.optString("id", null));
            topicsInfo.setTitle(jsonResult.optString("title", null));
            topicsInfo.setType(jsonResult.optString("type", null));
            topicsInfo.setAuthorPhoto(jsonResult.optString("authorPhoto", null));
            topicsInfo.setThumbUrl(jsonResult.optString("thumbUrl", null));
            topicsInfo.setCreateDate(jsonResult.optLong("createDate", 0));
            topicsInfo.setTags(tags);
            topicsInfo.setVideos(videos);
            topicsInfo.setAuthorId(jsonResult.optString("authorId", null));
            topicsInfo.setAuthor(jsonResult.optString("author", null));
        }
    ...
    @Override
    public TopicsImageTextInfo getTopicsImageTextInfo(String topicsId) throws NetworkDisconnectException, IOException, JSONException {
        if (!NailStarApplicationContext.getInstance().isNetworkConnected()) {
            throw new NetworkDisconnectException("网络没有连接");
        }
        ...
        String url = "/vapi/nailstar/topics/article/" + topicsId;
        //HttpClient.getJson(url);
        TopicsImageTextInfo topicsImageTextInfo = null;

        if (null != strVideoImageTextInfo && strVideoImageTextInfo.length() > 0) {
            topicsImageTextInfo = new TopicsImageTextInfo();
            ...
            topicsImageTextInfo.setId(jsonResult.optString("id", null));
            topicsImageTextInfo.setArticles(articles);
            topicsImageTextInfo.setPictures(pictures);
        }

        return topicsImageTextInfo;
    }

    @Override
    public ArrayList<TopicsRelatedInfo> getTopicsRelatedInfoList(String topicsId) throws NetworkDisconnectException, JSONException {
        if (!NailStarApplicationContext.getInstance().isNetworkConnected()) {
            throw new NetworkDisconnectException("网络没有连接");
        }
        ...
        String url = "/vapi/nailstar/topics/" + topicsId;
        //HttpClient.getJson(url);
        ArrayList<TopicsRelatedInfo> result = new ArrayList<TopicsRelatedInfo>();

        if (null != strTopicsRelatedInfo && strTopicsRelatedInfo.length() > 0) {
            TopicsRelatedInfo topicsRelatedInfo = new TopicsRelatedInfo();
            JSONObject jsonObject = new JSONObject(strTopicsRelatedInfo);
            JSONObject jsonResult = jsonObject.optJSONObject("result");

            JSONArray jsonRelated = jsonResult.optJSONArray("related");
            for (int i = 0; i < jsonRelated.length(); i++) {
                topicsRelatedInfo.setThumbUrl(jsonRelated.optJSONObject(i).optString("thumbUrl", null));
            }

            result.add(topicsRelatedInfo);
        }
