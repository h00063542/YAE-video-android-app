...
public class TopicServiceImpl implements ITopicService {
    @Override
    public TopicInfo getTopicInfo(String topicId) throws IOException, JSONException, NetworkDisconnectException {
        ...
        if (null != strVideoInfo && strVideoInfo.length() > 0) {
            ...
            topicInfo.setId(jsonResult.optString("id", null));
            topicInfo.setTitle(jsonResult.optString("title", null));
            topicInfo.setType(jsonResult.optString("type", null));
            topicInfo.setAuthorPhoto(jsonResult.optString("authorPhoto", null));
            topicInfo.setThumbUrl(jsonResult.optString("thumbUrl", null));
            topicInfo.setCreateDate(jsonResult.optLong("createDate", 0));
            topicInfo.setTags(tags);
            topicInfo.setVideos(videos);
            topicInfo.setAuthorId(jsonResult.optString("authorId", null));
            topicInfo.setAuthor(jsonResult.optString("author", null));
        }
    ...
    @Override
    public TopicImageTextInfo getTopicImageTextInfo(String topicId) throws NetworkDisconnectException, IOException, JSONException {
        if (!NailStarApplicationContext.getInstance().isNetworkConnected()) {
            throw new NetworkDisconnectException("网络没有连接");
        }
        ...
        if (null != strVideoImageTextInfo && strVideoImageTextInfo.length() > 0) {
            ...
            topicImageTextInfo.setId(jsonResult.optString("id", null));
    ...
    @Override
    public ArrayList<TopicRelatedInfo> getTopicRelatedInfoList(String topicId) throws NetworkDisconnectException, JSONException {
        if (!NailStarApplicationContext.getInstance().isNetworkConnected()) {
            throw new NetworkDisconnectException("网络没有连接");
        }
        ...
        if (null != strTopicsRelatedInfo && strTopicsRelatedInfo.length() > 0) {
            ...
            for (int i = 0; i < jsonRelated.length(); i++) {
                topicRelatedInfo.setThumbUrl(jsonRelated.optJSONObject(i).optString("thumbUrl", null));
            }
    ...
    @Override
    public ArrayList<TopicCommentInfo> getTopicComments(String topicId, int page) throws NetworkDisconnectException, JSONException {
        if (!NailStarApplicationContext.getInstance().isNetworkConnected()) {
            throw new NetworkDisconnectException("网络没有连接");
        }
        ...
        if (null != strTopicCommentsInfo && strTopicCommentsInfo.length() > 0) {
            ...
            for (int i = 0; i < jsonComments.length(); i++) {
                ...
                topicCommentInfo.setId(jsonComment.optString("id", null));
                topicCommentInfo.setUserId(jsonComment.optString("userid", null));
                topicCommentInfo.setAuthor(jsonComment.optString("author", null));
                topicCommentInfo.setAuthorPhoto(jsonComment.optString("authorPhoto", null));
                topicCommentInfo.setContent(jsonComment.optString("content"));
                topicCommentInfo.setContentPic(jsonComment.optString("contentPic", null));

                topicCommentInfo.setCreateDate(jsonComment.optLong("createDate", 0));
                topicCommentInfo.setIsHomework(jsonComment.optInt("isHomework", 0));
                topicCommentInfo.setIsMine(jsonComment.optInt("isMine", 0));
                topicCommentInfo.setStatus(jsonComment.optInt("status", 0));

                JSONObject jsonAt = jsonComment.optJSONObject("at");
                TopicCommentAtInfo topicCommentAtInfo = new TopicCommentAtInfo();
                topicCommentAtInfo.setUserId(jsonAt.optString("userId", null));
                topicCommentAtInfo.setNickName(jsonAt.optString("nickname", null));
                topicCommentInfo.setAt(topicCommentAtInfo);

                topicCommentInfo.setReplies(buildTopicCommentReplies(jsonComment.optJSONArray("replys")));
    ...
    private ArrayList<TopicCommentReplyInfo> buildTopicCommentReplies(JSONArray jsonReplies) {
        ...
        for (int i = 0; i < jsonReplies.length(); i++) {
            ...
            topicCommentReplyInfo.setId(jsonReply.optString("id", null));
            topicCommentReplyInfo.setUserId(jsonReply.optString("userid", null));
            topicCommentReplyInfo.setAuthor(jsonReply.optString("author", null));
            topicCommentReplyInfo.setContent(jsonReply.optString("content", null));
            topicCommentReplyInfo.setContentPic(jsonReply.optString("contentPic", null));
            topicCommentReplyInfo.setCreateDate(jsonReply.optLong("createDate", 0));
            topicCommentReplyInfo.setIsHomework(jsonReply.optInt("isHomework", 0));
            topicCommentReplyInfo.setStatus(jsonReply.optInt("status", 0));
